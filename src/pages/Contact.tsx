...
// You might need to install these type definitions to make TypeScript happy.
// npm i --save-dev @types/nodemailer @types/busboy
...
const Contact = () => {
    // This hook allows the component to access the current URL location.
  ...
  // Only append file(s) if selected
  if (fileInputRef.current?.files.length > 0) {
    for (let i = 0; i < fileInputRef.current.files.length; i++) {
      fd.append("files", fileInputRef.current.files[i]);
    }
  }
  ...
  try {
    const res = await fetch("/api/send-mail", {
      method: "POST",
      body: fd, // ✅ No Content-Type header here!
    });
